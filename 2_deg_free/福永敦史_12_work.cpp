...
void inverseKinematics(double x, double y, double& theta1, double& theta2) {
  ...
  for (int i = 0; i < maxIterations; i++) {
    ...
    // Forward kinematics to compute the end effector position
    double xEnd = L1 * cos(theta1) + L2 * cos(theta1 + theta2);
    double yEnd = L1 * sin(theta1) + L2 * sin(theta1 + theta2);

    // Compute the error between the desired position and the end effector position
    double errorX = x - xEnd;
    double errorY = y - yEnd;

    // 誤差が小さくなったらループを抜ける
    if (fabs(errorX) < epsilon && fabs(errorY) < epsilon) {
    ...
    theta1 += invJ11 * errorX + invJ12 * errorY;
    theta2 += invJ21 * errorX + invJ22 * errorY;
  }
}

// PD制御を使用して関節トルクを計算する関数
void calculateJointTorques(double theta1, double theta2, double& tau1, double& tau2, double prevTheta1, double prevTheta2) {
...
int main() {
  ...
  for (int i = 0; i < numPoints; i++) {
    ...
    double tau1, tau2;
    calculateJointTorques(theta1, theta2, tau1, tau2, prevTheta1, prevTheta2);
